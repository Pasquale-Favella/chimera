...
if (!env.GEMINI_API_KEY) {
	console.warn(
		"GEMINI_API_KEY not set. AI features will not work. Please set GEMINI_API_KEY in your .env file.",
	);
}
...
const designTokenSchema = z.object({
	colors: z.object({
		background: z.array(z.string()).describe("Background colors found in the design."),
	...
	typography: z.object({
		headingFont: z.string().describe("The primary font family used for headings."),
		bodyFont: z.string().describe("The primary font family used for body text."),
	}),
	borderRadius: z.array(z.string()).describe("Border radius values (e.g., 'rounded-lg')."),
	boxShadow: z.array(z.string()).describe("Box shadow values (e.g., 'shadow-md')."),
});
...
function buildMessages(
...
): Array<{ role: "user"; content: string | Array<{ type: "text"; text: string } | { type: "image"; image: string }> }> {
	...
	images.forEach((image) => {
		content.push({
			type: "image",
			image: image.base64, // Vercel AI SDK expects data URLs
		});
	});
