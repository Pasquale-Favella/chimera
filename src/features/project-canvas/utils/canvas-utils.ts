...
const getJsonSize = (
	...
	const height =
		typeof maybe.height === "number" ? maybe.height : fallback.height;
	return { width, height };
};
...
const mapTokens = (value: unknown): DesignTokens | null => {
	...
	return {
		colors: {
			background: Array.isArray(maybe.colors.background)
		...
		typography: {
			headingFont:
				typeof maybe.typography.headingFont === "string"
					? maybe.typography.headingFont
					: "",
			bodyFont:
				typeof maybe.typography.bodyFont === "string"
					? maybe.typography.bodyFont
					: "",
		},
